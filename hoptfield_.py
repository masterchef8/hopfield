...
class Hopfield(object):
    ...
    def evoluer(self):
        ...
        for i in self.pattern:
            ...
            #_ = np.array(pattern,float).reshape(5,5)
            #pl.imshow(_, interpolation='nearest', cmap=pl.cm.binary)
            #pl.show()

    def stable(self,data,pat):
        """
        Détermine si la valeur de sortie correspond à la valeur stockée en mémoire
        :param data: valeur de sortie
    ...
    def pas(self):
        ...
        for _ in self.pattern:
            ...
            print("le pattern ", _, "=> ", self.stable(_, np.sign(out)))
